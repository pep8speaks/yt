...
class YTDataChunk(object):
    ...
    def __init__(self, dobj, chunk_type, objs, data_size = None,
                 field_type = None, cache = False, indexer = None):
        self.dobj = dobj
    ...
    def _accumulate_values(self, method):
        ...
        arrs = []
        for obj in self._indexer or self.objs:
            f = getattr(obj, mname)
    ...
    def fcoords(self):
        if self._indexer is not None:
            ci = self._indexer.select_fcoords(
    ...
    def icoords(self):
        if self._indexer is not None:
            ci = self._indexer.select_icoords(
                self.dobj.selector, self.data_size)
            return ci
    ...
    def fwidth(self):
        if self._indexer is not None:
            ci = self._indexer.select_fwidth(
                self.dobj.selector, self.data_size)
            ci = YTArray(ci, input_units = "code_length",
                         registry = self.dobj.ds.unit_registry)
            return ci
    ...
    def dtcoords(self):
        ...
        ind = 0
        # Until we implement an indexer version, use this
